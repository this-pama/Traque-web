...
const filters = [
    {
        ...
        amountLabel: 'FILE',
        comparator: (rowData) =>
            rowData.pending && rowData.pending.value
    },
    {
        title: 'Delayed',
        role: 'Type',
        amountLabel: 'FILE',
        comparator: (rowData) =>
        rowData.delayed && rowData.delayed.value
    },
    {
        title: 'Archived',
        role: 'Type',
        amountLabel: 'FILE',
        comparator: (rowData) =>
        rowData.archived && rowData.archived.value
    },
    {
        title: 'Exceed SLA',
        role: 'Type',
...

    {
        ...
        amountLabel: 'FILE',
        comparator: (rowData) =>
        rowData && rowData.archived.value == false
...
const View = ({props}) => {
    ...
    const endpoint = `/v1/file/ministry/${user && user.ministry}`
    ...
    return (
        ...
        <TableView
            title={'Ministry File Report'}
            data={applications}
            filters={filters}
            fetchData={fetchData}
            gridConfig={getColumnDefs('sectionReport', fetchData)}
            exportFileName={'Ministry files'}
        />
        </>
    )
}
