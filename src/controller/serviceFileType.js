...
export default({ config, db }) => {
  ...
  api.post('/add/:userId', async (req,res) =>{
    ...
    if(isObjectIdValid(userId) == false) return res.status(500).send("User id is invalid");
    ...
    let service = ServiceFile({name, userId: user._id , ministry: user.ministry })
  ...
  api.put('/update/:id', async (req, res)=>{
    ServiceFile.findByIdAndUpdate(req.params.id, { name: req.body.name }, {new: true})
