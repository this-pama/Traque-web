    ...
    (req, res, next) => {
      Account.findOne({ email: req.body.email }, (err, account) => {
        ...
        if (account && account.disable)
          return res.status(200).json({ message: "Account has been disabled" });
    ...
    async.waterfall(
      [
        ...
        function (token, user, done) {
          ...
          sendMail(
            "donotreply-passwordreset@aapexapps.net",
            {
              email: user.email,
  ...
  api.get("/reset/:token", function (req, res) {
    Account.findOne(
      ...
      function (err, user) {
        ...
        return res
          .status(200)
          .json({ id: user._id, resetPasswordToken: user.resetPasswordToken });
      }
    );
  });
