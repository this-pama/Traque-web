...
export const SLAJob = async()=>{
    ...
    const message = p => `Dear ${p.firstName}, The file ${p.fileName} will default the SLA time in the next one hour. Kindly attend to the file as soon as possible.`
    ...
    .then(e=> {
        ...
        e.map(p=>{
            if(p._id){
                ...
                sendMail( EMAIL_SENDER , { email: data.email, firstName: data.firstName, message: message(data) }, 'file');
            }
